...
@Entity
@Table(name = "p_limited_product_stock")
@Getter
@NoArgsConstructor(access = AccessLevel.PROTECTED)
public class Limited_Product_Stock extends BaseCreateEntity {
	...
	@Builder
	public Limited_Product_Stock(UUID limitedproductId, int quantity) {
		// 재고 증감량은 0보다 커야 함
		if (quantity < 0) {
	...
	// ⭐ 한정상품 재고 생성 정적 메서드
	public static Limited_Product_Stock createStock(UUID limitedProductId, int quantity) {
		return Limited_Product_Stock.builder()
			.limitedproductId(limitedProductId)
			.quantity(quantity)
			.build();
	}
}
